...
def convert_kins_json_to_aistron_format(kins_ann_file):
    kins_ann_dict = json.load(open(kins_ann_file, 'r'))
    for ann in kins_ann_dict['annotations']:
        ann['amodal_segm'] = ann.pop('segmentation')
        ann['amodal_bbox'] = ann.pop('bbox')
        ann['visible_segm'] = ann.pop('inmodal_seg')
        ann['visible_bbox'] = ann.pop('inmodal_bbox')

    tmp_json_fname = os.path.dirname(kins_ann_file)
    tmp_json_fname = os.path.join(tmp_json_fname, 'tmp.json')
    with open(tmp_json_fname, 'w') as f:
        json.dump(kins_ann_dict, f)

    return tmp_json_fname, kins_ann_dict['categories']
...
if __name__ == "__main__":
    ...
    tmp_json_fname, categories = convert_kins_json_to_aistron_format(json_ann_path)
